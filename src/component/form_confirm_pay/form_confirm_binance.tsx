...
export default function FormConfirmBinance ({ monto, callConfirmdata }: { monto: number, callConfirmdata: (e: dataFormShow) => {} }) {
    const [thisDate, setthisDate] = useState<any>()
    const [records, setRecords] = useState<dataFormShow>({ idu: 0, formatpay: 0, bank_config: undefined, code_confirm: "not-referent", date_transac: "", monto: monto, name_confirm: "" })
    ...
    return (
        ...
        <form onSubmit={handleConfirm} className="grid gap-4 mb-4 sm:grid-cols-2 ">
            <div>
                <label htmlFor="num_confirm" className="block  mb-2 text-sm font-medium text-gray-900 dark:text-white">Correo Emisor:</label>
                <input required onChange={({ target: { value } }) => handlenDataform(1, value)} type="email" name="num_confirm" id="num_confirm" className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-600 focus:border-blue-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="correo@company.com" />
            </div>
            <div>
