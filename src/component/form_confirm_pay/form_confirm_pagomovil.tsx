...
export default function FormConfirmPagoMovil ({ monto, callConfirmdata }: { monto: number, callConfirmdata: (e: dataFormShow) => {} }) {
    const [listBank, setListBank] = useState<dataListBank[]>()
    const [thisDate, setthisDate] = useState<any>()
    const [records, setRecords] = useState<dataFormShow>({ idu: 0, bank_code: 0, formatpay: 0, bank_config: "No Selecionado", code_confirm: "", date_transac: "", monto: monto, name_confirm: undefined })
