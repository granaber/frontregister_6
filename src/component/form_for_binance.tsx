...
const IDBINANCE = ''
const ANOMBRE = 'Leomar1995urdaneta@gmail.com'

export default function FormForBinance ({ monto, callProcessPay }: { monto: number, callProcessPay: (e: keyTypeForm) => {} }) {
    ...
    return (
        <div className="flex flex-col items-center mx-2 mt-1">
            <div className="mt-4">
                <BinanceSvg className="w-[60px] h-[60px]  inline-block " />
            </div>
            <div className="mt-4 border rounded-md p-1 w-full hidden">
                <label htmlFor="tdc" className="inline-block mb-1  mr-2 text-base  font-medium text-sky-900 dark:text-white">Id Binance </label>
                <h3 className="inline" >{IDBINANCE} <button onClick={() => settext(IDBINANCE)}><IconCopy className="my-2 inline-block" /></button></h3>
            </div>

            <div className="mt-4 border rounded-md p-1 w-full">
                <label htmlFor="tdc" className="inline-block mb-1  mr-2 text-base  font-medium text-sky-900 dark:text-white">Usuario</label>
